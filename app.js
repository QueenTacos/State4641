...
const ADMIN_TABS = [
  ...
  { id: "nap", label: "NAP DASHBOARD", adminOnly: true },
...
function renderAllianceNotificationsPanelHtml(user, lockedAlliance) {
...
function wireAllianceNotificationsPanel(el, user, lockedAlliance) {
...
function renderAdmin(el) {
  ...
  el.innerHTML = `
    ...
    ${adminActiveTab !== "nap" || officerScoped ? "" : renderNapAdminPanelHtml(user)}
...
function renderAllianceDashboardTabHtml(user, officerScoped) {
  ...
  return `
    ...
    ${allianceDashSubTab !== "discipline" ? "" : renderAllianceDashDisciplineHtml(viewingAlliance)}
  `;
}
  ...
  return `
    <div class="panel">
      ...
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px;">
        ${
          ...
          <div style="display:flex;gap:10px;align-items:flex-start;background:var(--panel-2);border:1px solid var(--border);border-radius:4px;padding:8px 10px;flex-wrap:wrap;">
            ...
            <button data-etdel="${t.id}" class="btn small" style="color:var(--accent-red);">✕</button>
          </div>`
                )
                .join("")
      ...
      <div class="field-row">
        ...
        <div class="field"><label>TIME</label><input id="etTime" placeholder="e.g. 20:00 UTC daily" /></div>
...
function renderAllianceDashDisciplineHtml(viewingAlliance) {
...
function wireAllianceDashboardTab(el, user, officerScoped) {
  ...
  if (allianceDashSubTab === "notifications") wireAllianceNotificationsPanel(el, user, viewingAlliance);
}
...
function napDashboardAccessible(user) {
  ...
  const tag = user.alliance;
  return !!(tag && Store.napAlliances[tag]?.isNap);
}
...
// Public "/nap" viewer — read-only. Order per spec: rules, rankings,
// current fortress, current stronghold, previous fortress, previous
// stronghold (history sections collapsible via <details>).
function renderNapDashboard(el) {
  ...
  if (!napDashboardAccessible(user)) {
    el.innerHTML = `
      ...
      <div class="panel gate">
        <p>${user ? "This page is only visible to alliances currently included in the Non-Aggression Pact." : t("admin.leadershipOnly")}</p>
