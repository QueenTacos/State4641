...
const ROUTES = {
  ...
  "/nap": renderNapDashboard,
  "/alliance": renderAllianceDashboardStandalone,
};
...
// uses "officer" internally for the R4 role (role checks, filters, etc. all
// stay as-is) — this only changes what shows up in the UI, matching the
// alliance-rank naming ("R4") the game itself uses. "leader" is its own,
// separate role value both internally and in the UI — see ROLE SYSTEM below.
...
//
//   isAdmin(user)          — can see the Admin page at all (data.js).
//   allianceScoped(user)   — true for LEADER or R4: full alliance-level
//                            management, but locked to their own alliance
//                            (Store.currentUser.alliance) everywhere the
//                            Admin page shows data. Used exactly where the
//                            old `officerScoped` boolean was used — LEADER
//                            gets every power R4 already had, plus R4
//                            management (see canManageR4Roles below).
//   canManageR4Roles(user) — ADMIN or LEADER only. Governs the "R4
//                            Management" block in the Alliance Dashboard
//                            (promote Member→R4 / demote R4→Member) — R4
//                            itself must NOT get this, even though R4 is
//                            otherwise alliance-scoped just like LEADER.
//   userAllianceTag(user)  — the alliance a LEADER/R4 is locked to, or null
//                            for ADMIN (who isn't tied to one) / MEMBER.
// ---------------------------------------------------------------------------
function allianceScoped(user) {
  return !!user && (user.role === "officer" || user.role === "leader");
}
function canManageR4Roles(user) {
  return !!user && (user.role === "admin" || user.role === "leader");
}
function userAllianceTag(user) {
  return allianceScoped(user) ? user.alliance || null : null;
}
...
function renderShell() {
  ...
  document.getElementById("shell").innerHTML = `
    ...
    <nav class="bottom-nav">
      ...
      ${napDashboardAccessible(user) ? `<a href="#/nap" data-path="/nap">☮<br/>NAP</a>` : ""}
    ...
    <div class="ops-grid">
      ...
      ${PLANNED_TOOLS.map((tool, i) =>
        opCard({
          ...
          num: String(i + 10).padStart(2, "0"), iconName: tool.icon || "doc", tag: tool.tag, scene: tool.scene || "bear_calculator",
        })
      ).join("")}
    </div>
...
// "notifications" moved off this top-level list — Alliance Notifications
// now lives as a sub-tab inside "alliance-dash" (see below), scoped per
// alliance. "alliance-dash" and "nap" are new: "alliance-dash" is visible
// to everyone who reaches the Admin page at all (ADMIN/LEADER/R4 — MEMBER
// never gets here, isAdmin() already excludes them) since LEADER/R4 manage
// their own alliance there; "nap" stays `adminOnly` — NAP Dashboard
// management (rules/rankings/fortress/stronghold) is state-wide, full-Admin
// only, same tier as State Settings.
const ADMIN_TABS = [
...
// LEADER/R4 never sees "State Dashboard" at all (their existing flat
// Members/SVS/Alliance Dashboard/NAP Dashboard tabs are unchanged), since
// State Dashboard is admin-only per spec.
let adminSection = "state"; // "state" | "nap" | "alliance"

// Alliance Dashboard's own inner sub-tab bar (see renderAllianceDashboardTabHtml
...
let allianceDashSubTab = "overview";
// Which alliance's dashboard a full ADMIN is currently viewing (LEADER/R4
// are hard-locked to their own — see userAllianceTag() above). Same pattern
// as champViewingAlliance for the Championship page.
let allianceDashViewingAlliance = null;

// NAP Dashboard's own admin-management sub-tab bar (Admin → NAP DASHBOARD
// tab only — see renderNapAdminPanelHtml below).
const NAP_ADMIN_SUBTABS = [
  { id: "rules", label: "NAP Rules" },
  { id: "alliances", label: "NAP Alliances" },
  { id: "fortress-current", label: "Fortress Sign Ups" },
  { id: "fortress-history", label: "Fortress History" },
  { id: "stronghold-current", label: "Stronghold Sign Ups" },
  { id: "stronghold-history", label: "Stronghold History" },
];
let napAdminSubTab = "rules";
...
// ---------------------------------------------------------------------------
// `lockedAlliance` (null for a full ADMIN) is the alliance tag a LEADER/R4
// is locked to when this panel is embedded in their Alliance Dashboard —
// it forces both the create-form's alliance and the list filter to that
// one alliance (disabling the pickers rather than hiding them, so it's
// still obvious which alliance this is), and every write below re-checks
// it defensively so a LEADER/R4 can't act on another alliance's notices
// even via a crafted request.
function renderAllianceNotificationsPanelHtml(user, lockedAlliance, canManage) {
  ...
  const formAlliance = lockedAlliance || seed?.allianceTag || "";
  const formEventId = seed?.eventId || "";
  const formText = seed?.noticeText || "";

  const f = { ...allianceNoticeFilters, alliance: lockedAlliance || allianceNoticeFilters.alliance };
  ...
  return `
    ...
    <div class="panel">
      ...
      <p style="font-size:11.5px;color:var(--text-dim);margin-top:-6px;">Write a reusable notice for an alliance and an event, then copy it straight into chat.${lockedAlliance ? ` Scoped to ${escapeHtml(lockedAlliance)} — only Admin manages other alliances' notices.` : ""}</p>

      <div class="section-title" style="margin-top:16px;">${editing ? "EDIT NOTICE" : "CREATE NOTICE"}</div>
      <div class="field-row">
        <div class="field">
          <label>ALLIANCE</label>
          <select id="anAlliance" ${lockedAlliance ? "disabled" : ""}>
            ${lockedAlliance ? "" : `<option value="">Select alliance…</option>`}
    ...
    <div class="panel">
      ...
      ${!canManage ? `<p style="font-size:11px;color:var(--text-dim);margin:-4px 0 10px;">Read-only — only Admin, this alliance's Leader, or R4 can create, edit, or delete notices.</p>` : ""}
      <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px;">
        <select id="anFAlliance" ${lockedAlliance ? "disabled" : ""} style="background:var(--panel-2);border:1px solid var(--border);color:var(--text);border-radius:4px;padding:6px 8px;font-size:11.5px;">
          ${lockedAlliance ? "" : `<option value="">All alliances</option>`}
          ${alliances.map((a) => `<option value="${escapeHtml(a)}" ${f.alliance === a ? "selected" : ""}>${escapeHtml(a)}</option>`).join("")}
        </select>
...
function wireAllianceNotificationsPanel(el, user, lockedAlliance, canManage) {
  ...
  el.querySelector("#anSave")?.addEventListener("click", () => {
    // Defense in depth — the create/edit form itself is never rendered for
    // a non-manager (see renderAllianceNotificationsPanelHtml's `canManage`
    // gate), so #anSave normally doesn't exist to attach this to at all;
    // this still blocks the write outright if it's ever reached anyway.
    if (!canManage) return;
    const errEl = el.querySelector("#anErr");
    const allianceTag = lockedAlliance || el.querySelector("#anAlliance").value;
    ...
    // Defense in depth — a LEADER/R4 can't be tricked into (or accidentally
    // end up) saving a notice under a different alliance than they're
    // locked to, even though the alliance <select> is already disabled to
    // lockedAlliance in the UI above.
    if (lockedAlliance && allianceTag !== lockedAlliance) { errEl.textContent = "You can only manage notices for your own alliance."; return; }
  ...
  el.querySelector("#anFAlliance")?.addEventListener("change", (e) => {
    if (lockedAlliance) return; // disabled in the UI; belt-and-suspenders
  ...
  el.querySelectorAll("[data-anedit]").forEach((btn) =>
    btn.addEventListener("click", () => {
      if (!canManage) return;
      const n = Store.allianceNotices.find((x) => x.id === btn.dataset.anedit);
      if (lockedAlliance && n && n.allianceTag !== lockedAlliance) return; // can't happen via the UI — only ever rendered for this alliance's own notices — but checked anyway
  ...
  el.querySelectorAll("[data-andup]").forEach((btn) =>
    btn.addEventListener("click", () => {
      ...
      if (!n) return;
      if (lockedAlliance && n.allianceTag !== lockedAlliance) return;
  ...
  el.querySelectorAll("[data-andel]").forEach((btn) =>
    btn.addEventListener("click", () => {
      ...
      const n = Store.allianceNotices.find((x) => x.id === id);
      if (lockedAlliance && n && n.allianceTag !== lockedAlliance) return;
...
function renderAdmin(el) {
  ...
  // LEADER and R4 only manage their own alliance's roster here — the full
  // admin role still sees everyone. This scopes both the Members table and
  // the Bag submissions table below, since both are built from `members`.
  // Kept as the name `officerScoped` (rather than renamed everywhere) to
  // minimize the diff — LEADER now shares every one of R4's existing
  // alliance-scoped powers on this page (see allianceScoped() above); the
  // one thing LEADER additionally gets — R4 role management — is gated
  // separately by canManageR4Roles(), not by this variable.
  const officerScoped = allianceScoped(user);
  const members = officerScoped ? allMembers.filter((m) => m.alliance === user.alliance) : allMembers;
  const alliances = Store.alliances;
  const furnaceFc = Store.furnaceFc;
  const bagSubs = Store.bagSubmissions;

  // True ADMIN gets the restructured 3-section nav — State Dashboard / NAP
  // Dashboard / Alliance Dashboard, State Dashboard nesting the rest of
  // ADMIN_TABS as its own sub-tab bar. LEADER/R4 keep today's flat single-
  // level tab bar unchanged (they never had a "State Dashboard" grouping to
  // begin with — Members/SVS Battle Sign Ups/Alliance Dashboard were always
  // their own alliance-scoped tabs, now joined by the new read-only NAP tab).
  const isTrueAdmin = user.role === "admin";

  // Visible tabs depend on role — an officer only ever sees the same
  // (now four, with NAP added) sections that already rendered for them
  ...
  if (isTrueAdmin) {
    if (!["state", "nap", "alliance"].includes(adminSection)) adminSection = "state";
    // Keep adminActiveTab in sync with whichever primary section is active,
    // so the existing per-tab dispatch conditionals below (unchanged) still
    // pick the right body — NAP/Alliance Dashboard are single sections with
    // no sub-tabs of their own, State Dashboard keeps its existing sub-tabs.
    if (adminSection === "nap") adminActiveTab = "nap";
    else if (adminSection === "alliance") adminActiveTab = "alliance-dash";
  ...
  el.innerHTML = `
    ...
    ${
      isTrueAdmin
        ? `
    <div class="pill-toggle" style="max-width:560px;">
      <button data-admsection="state" class="${adminSection === "state" ? "active" : ""}">STATE DASHBOARD</button>
      <button data-admsection="nap" class="${adminSection === "nap" ? "active" : ""}">NAP DASHBOARD</button>
      <button data-admsection="alliance" class="${adminSection === "alliance" ? "active" : ""}">ALLIANCE DASHBOARD</button>
    </div>
    ${
      adminSection === "state"
        ? `${dashboardHeroHtml(
                ...
                <td>
                  ${
                    m.permanent
                      ...
                      : `<select data-mfield="role" data-mid="${m.id}" ${m.id === user.id ? `disabled title="You can't change your own role — ask another admin."` : ""} style="background:var(--panel-2);border:1px solid var(--border);color:var(--text);border-radius:3px;padding:5px 8px;font-size:12px;">
                          ${["member", "officer", "leader", "admin"].map((r) => `<option value="${r}" ${m.role === r ? "selected" : ""}>${roleLabel(r)}</option>`).join("")}
                        </select>`
                  }
                </td>
    ...
    ${
      adminActiveTab !== "nap"
        ? ""
        : isTrueAdmin
        ? renderNapAdminPanelHtml(user)
        : `<div class="panel"><p style="font-size:12px;color:var(--text-dim);">Read-only — only ADMIN can edit the NAP Dashboard.</p></div>${renderNapDashboardBodyHtml()}`
    }
  ...
  el.querySelectorAll("[data-admsection]").forEach((btn) =>
    btn.addEventListener("click", () => {
      adminSection = btn.dataset.admsection;
      renderAdmin(el);
    })
  );
  ...
  el.querySelectorAll("[data-mfield]").forEach((input) =>
    input.addEventListener("change", () => {
      ...
      if (idx === -1) return;
      // Prevent accidental lockout: an alliance can't be left with zero
      // LEADERs by demoting/reassigning its only one. Only checked when the
      // role is actually changing away from "leader" — promoting someone
      // else to leader, or any other field edit, is unaffected. The
      // role <select> is also `disabled` on your own row (see above) so an
      // admin can't demote themselves out from under this same way.
      if (field === "role" && m[idx].role === "leader" && input.value !== "leader") {
        const remainingLeaders = m.filter((mm) => mm.id !== id && mm.role === "leader" && mm.alliance === m[idx].alliance);
        if (!remainingLeaders.length && m[idx].alliance) {
          alert(`${m[idx].alliance || "This alliance"} would be left with no LEADER. Promote another member to LEADER first.`);
          input.value = "leader";
          return;
        }
      }
      ...
      // A LEADER/R4 editing a member's alliance can move that member out of
      // (or into) their own filtered view — re-render so the table reflects
...
// ---------------------------------------------------------------------------
// ALLIANCE DASHBOARD — a new Admin tab, visible to ADMIN/LEADER/R4 (not
// MEMBER — the whole Admin page already gates that). LEADER/R4 are locked
// to their own alliance (userAllianceTag()); ADMIN picks which alliance to
// view via a dropdown, same pattern as Alliance Championship's
// champViewingAlliance. Everything inside is either a MOVE of an existing
// feature (Notifications — see renderAllianceNotificationsPanelHtml above,
// now alliance-locked) or DERIVED from existing data (Participation,
// Performance — no new manual-entry forms, per spec); Event Times and
// Discipline are the two genuinely new, lightweight record types (see
// Store.allianceEventTimes / Store.allianceDiscipline in data.js).
// ---------------------------------------------------------------------------

function allianceDashboardViewingAlliance(user, officerScoped) {
  if (officerScoped) return user.alliance || null;
  return allianceDashViewingAlliance || Store.alliances[0] || null;
}

// Standalone "/alliance" route — the home-page "Alliance Dashboard" card's
// destination for any signed-in MEMBER/LEADER/R4 (ADMIN normally uses
// Admin → Alliance Dashboard's cross-alliance picker instead, but can reach
// this page too if they personally belong to an alliance). Always locked to
// the signed-in user's OWN alliance (officerScoped forced to `true` here
// regardless of actual role) — no alliance picker, ever, on this route; see
// section 5/6 of the access spec ("do not ask a Member/Leader/R4 to choose
// an alliance... must not be able to access another alliance"). Edit
// controls still only appear where the user's actual role earns them
// (isAdmin(user)/canManageR4Roles(user) inside the sub-renderers below are
// unaffected by the forced `officerScoped=true` — see the code comment on
// wireAllianceDashboardTab's officerScoped param).
function renderAllianceDashboardStandalone(el) {
  const user = Store.currentUser;
  if (!user) {
    el.innerHTML = `
...
function renderAllianceDashboardTabHtml(user, officerScoped) {
  const alliances = Store.alliances;
  if (officerScoped && !user.alliance) {
    return `
    <div class="panel gate">
      <p>Your account isn't assigned to an alliance yet.</p>
      <p style="font-size:12px;">Ask an admin to set your alliance in Admin → Members — the Alliance Dashboard is scoped per alliance, so LEADER/R4 access needs an alliance tag first.</p>
    </div>`;
  }
  ...
  if (!viewingAlliance) {
    ...
    <div class="panel gate">
      ...
      <p style="font-size:12px;">Add one in Admin → Alliances before using the Alliance Dashboard — everything here is scoped to a specific alliance.</p>
    </div>`;
  }

  const members = Store.members.filter((m) => m.alliance === viewingAlliance);
  const bagSubs = Store.bagSubmissions;
            ...
            ${
              ...
              <tr>
                ...
                <td style="text-align:right;">
                  ${
                    m.role === "member"
                      ? `<button data-r4make="${m.id}" class="btn small primary">Make R4</button>`
                      : m.role === "officer"
                      ? `<button data-r4remove="${m.id}" class="btn small" style="color:var(--accent-red);">Remove R4</button>`
                      : `<span style="font-size:10.5px;color:var(--text-faint);">—</span>`
                  }
                </td>
              </tr>`
...
function renderAllianceDashEventTimesHtml(viewingAlliance, canManage) {
  const times = (Store.allianceEventTimes[viewingAlliance] || []).slice().sort((a, b) => a.title.localeCompare(b.title));
  return `
    <div class="panel">
      <div class="planner-header"><strong>Event Times (${times.length})</strong></div>
      <p style="font-size:11.5px;color:var(--text-dim);margin-top:-6px;">A short reference list of this alliance's own recurring commitments — for date-based events with the full calendar, use Admin → Schedule / Events instead.</p>
        ...
        ${
          times.length
            ? times
                .map(
                  (t) => `
          <div style="display:flex;gap:10px;align-items:flex-start;background:var(--panel-2);border:1px solid var(--border);border-radius:4px;padding:8px 10px;flex-wrap:wrap;">
            <div style="flex:1 1 200px;min-width:0;">
              <div style="font-size:12.5px;font-weight:700;">${escapeHtml(t.title)}</div>
              <div style="font-size:11.5px;color:var(--accent-amber);">${escapeHtml(t.timeText)}</div>
              ${t.notes ? `<div style="font-size:11px;color:var(--text-faint);margin-top:2px;">${escapeHtml(t.notes)}</div>` : ""}
            </div>
            ${canManage ? `<button data-etdel="${t.id}" class="btn small" style="color:var(--accent-red);">✕</button>` : ""}
          </div>`
                )
                .join("")
            : `<div class="empty">No event times yet.</div>`
        }
      </div>
      ${
        canManage
          ...
          : `<p style="font-size:11px;color:var(--text-dim);margin:0;">Read-only — only Admin, this alliance's Leader, or R4 can add or remove event times.</p>`
      }
    </div>
  `;
}
...
function renderAllianceDashParticipationHtml(viewingAlliance, members, bagSubs, svsSignups, canManage) {
  ...
  return `
    <div class="panel">
      <div class="planner-header"><strong>Participation</strong></div>
      <p style="font-size:11.5px;color:var(--text-dim);margin-top:-6px;">Derived from existing bag submissions and SVS signups — nothing entered here manually.</p>
      <div class="grid2" style="margin-bottom:14px;">
        <div class="summary-row"><span class="k">Bag submission rate</span><span class="v">${bagRate}%</span></div>
        <div class="summary-row"><span class="k">SVS signup rate</span><span class="v">${svsRate}%</span></div>
      </div>
      <div style="overflow-x:auto;">
        <table>
...
function renderAllianceDashPerformanceHtml(viewingAlliance, members, bagSubs, canManage) {
  const ranked = members
    .map((m) => {
      const sub = bagSubs[m.id];
      const total = sub ? computeBagPoints(sub.values).total : 0;
      return { m, total, hasSub: !!sub };
    })
    .sort((a, b) => b.total - a.total);
  return `
    <div class="panel">
      <div class="planner-header"><strong>Performance</strong></div>
      <p style="font-size:11.5px;color:var(--text-dim);margin-top:-6px;">Ranked by projected SvS points from each member's submitted bag — same numbers as Admin → Members → Bag submissions.</p>
      <div class="rank-list">
        ${
          ranked
            .map(
          ...
          <div class="rank-item">
            <div class="left">
              <span class="name">#${i + 1} ${escapeHtml(r.m.name)}</span>
              <span class="desc">${r.hasSub ? "Bag submitted" : "No bag submission yet"}</span>
            </div>
            <span class="score">${fmtNum(r.total)}</span>
          </div>`
            )
...
function renderAllianceDashDisciplineHtml(viewingAlliance, canManage) {
  const log = (Store.allianceDiscipline[viewingAlliance] || []).slice().sort((a, b) => b.createdAt - a.createdAt);
  const members = Store.members.filter((m) => m.alliance === viewingAlliance);
  return `
    <div class="panel">
      <div class="planner-header"><strong>Discipline (${log.length})</strong></div>
      <p style="font-size:11.5px;color:var(--text-dim);margin-top:-6px;">A shared strike/warning log — no auto-enforcement, just a record LEADER/R4/Admin can see and add to.</p>
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px;">
        ${
          log.length
            ? log
          ...
          <div style="display:flex;gap:10px;align-items:flex-start;background:var(--panel-2);border:1px solid var(--border);border-radius:4px;padding:8px 10px;flex-wrap:wrap;">
            <div style="flex:1 1 220px;min-width:0;">
              <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:2px;">
                <strong style="font-size:12.5px;">${escapeHtml(e.memberName)}</strong>
            ...
            ${canManage ? `<button data-discdel="${e.id}" class="btn small" style="color:var(--accent-red);">✕</button>` : ""}
          </div>`
                )
                .join("")
            : `<div class="empty">No discipline entries yet.</div>`
        }
      </div>
      ${
        canManage
          ? `<div class="field-row">
              ...
              <div class="field">
                ...
                <select id="discSeverity">
                  ${DISCIPLINE_SEVERITIES.map((s) => `<option value="${s}">${s.toUpperCase()}</option>`).join("")}
                </select>
              </div>
              <div class="field" style="flex:1;min-width:220px;"><label>NOTE</label><input id="discNote" placeholder="What happened…" /></div>
            </div>
            <button class="btn small primary" id="discAdd">Add Entry</button>`
          : `<p style="font-size:11px;color:var(--text-dim);margin:0;">Read-only — only Admin, this alliance's Leader, or R4 can add or remove discipline entries.</p>`
      }
    </div>
  `;
}

function wireAllianceDashboardTab(el, user, officerScoped) {
  el.querySelector("#allianceDashSelect")?.addEventListener("change", (e) => {
    allianceDashViewingAlliance = e.target.value;
    router();
  });
  el.querySelectorAll("[data-adsubtab]").forEach((btn) =>
    btn.addEventListener("click", () => {
      allianceDashSubTab = btn.dataset.adsubtab;
      router();
    })
  );
  ...
  if (allianceDashSubTab === "calendar") {
    wireAllianceCalendarSection(el, viewingAlliance, isAdmin(user), () => router());
  }
  ...
  // handler re-checks the member's current alliance against
  // viewingAlliance anyway, so a LEADER can't act outside their own.
  el.querySelectorAll("[data-r4make]").forEach((btn) =>
    btn.addEventListener("click", () => {
      if (!canManageR4Roles(user)) return;
      const id = btn.dataset.r4make;
      const m = Store.members;
      const idx = m.findIndex((mm) => mm.id === id);
      if (idx === -1 || m[idx].alliance !== viewingAlliance || m[idx].role !== "member") return;
      m[idx].role = "officer";
      Store.members = m;
      router();
    })
  );
  el.querySelectorAll("[data-r4remove]").forEach((btn) =>
    btn.addEventListener("click", () => {
      if (!canManageR4Roles(user)) return;
      const id = btn.dataset.r4remove;
      const m = Store.members;
      const idx = m.findIndex((mm) => mm.id === id);
      if (idx === -1 || m[idx].alliance !== viewingAlliance || m[idx].role !== "officer") return;
      m[idx].role = "member";
      Store.members = m;
      router();
    })
  ...
  // Event Times — canManage (isAdmin(user)) guard is defense in depth: the
  // add/delete controls themselves are only ever rendered for a manager
  // (see renderAllianceDashEventTimesHtml), so these normally don't exist
  // to attach to at all for a plain MEMBER.
  el.querySelector("#etAdd")?.addEventListener("click", () => {
    if (!isAdmin(user)) return;
    const title = el.querySelector("#etTitle").value.trim();
    const timeText = el.querySelector("#etTime").value.trim();
    const notes = el.querySelector("#etNotes").value.trim();
    if (!title || !timeText || !viewingAlliance) return;
    const all = Store.allianceEventTimes;
    const list = all[viewingAlliance] || [];
    all[viewingAlliance] = [...list, { id: "et" + Date.now(), title, timeText, notes }];
    Store.allianceEventTimes = all;
    router();
  });
  el.querySelectorAll("[data-etdel]").forEach((btn) =>
    btn.addEventListener("click", () => {
      if (!isAdmin(user) || !viewingAlliance) return;
      const all = Store.allianceEventTimes;
      all[viewingAlliance] = (all[viewingAlliance] || []).filter((t) => t.id !== btn.dataset.etdel);
      Store.allianceEventTimes = all;
      router();
    })
  );

  // Discipline — same defense-in-depth pattern as Event Times above.
  el.querySelector("#discAdd")?.addEventListener("click", () => {
    if (!isAdmin(user)) return;
    const memberId = el.querySelector("#discMember").value;
    const severity = el.querySelector("#discSeverity").value;
    const note = el.querySelector("#discNote").value.trim();
    const member = Store.members.find((m) => m.id === memberId);
    if (!memberId || !member || !note || !viewingAlliance) return;
    const all = Store.allianceDiscipline;
    const list = all[viewingAlliance] || [];
    all[viewingAlliance] = [
      ...list,
      {
        ...
        note,
        severity: DISCIPLINE_SEVERITIES.includes(severity) ? severity : "note",
        createdBy: user?.id || null,
        createdByName: user?.name || "",
        createdAt: Date.now(),
      },
    ];
    Store.allianceDiscipline = all;
    router();
  });
  el.querySelectorAll("[data-discdel]").forEach((btn) =>
    btn.addEventListener("click", () => {
      if (!isAdmin(user) || !viewingAlliance) return;
      const all = Store.allianceDiscipline;
      all[viewingAlliance] = (all[viewingAlliance] || []).filter((e) => e.id !== btn.dataset.discdel);
      Store.allianceDiscipline = all;
      router();
    })
  );
...
// ---------------------------------------------------------------------------
// NAP DASHBOARD — state-level (not per-alliance), tracking who's in the
// Non-Aggression Pact, current rankings by Power, and Fortress/Stronghold
// signups. Two halves: renderNapAdminPanelHtml (Admin → NAP DASHBOARD tab,
// full-admin-only, all editing happens here) and renderNapDashboard (the
// public "/nap" viewer route — read-only for everyone who can see it,
// including LEADER/R4/MEMBER of a NAP-flagged alliance; see
// napDashboardAccessible below). See the "NAP Dashboard" block in data.js
// for the Store fields and normalizeNapSignup().
// ---------------------------------------------------------------------------

// Every signed-in user (ADMIN/LEADER/R4/MEMBER, regardless of their own
// alliance's NAP status) can VIEW the NAP Dashboard — only ADMIN can edit
// it (see renderNapAdminPanelHtml, reachable only from inside Admin).
function napDashboardAccessible(user) {
  return !!user;
}

// Rank is always DERIVED from `power` here — never stored — so editing
// Power can never leave a stale rank behind. Only alliances flagged
// isNap:true are included, and only alliances that still exist in
// Store.alliances (an alliance removed from Admin → Alliances silently
// drops out of the ranking rather than showing a dangling tag).
function napRankedAlliances() {
  const napMap = Store.napAlliances;
  return Store.alliances
    .filter((a) => napMap[a]?.isNap)
    .map((a) => ({ tag: a, ...napMap[a] }))
    .sort((a, b) => (b.power || 0) - (a.power || 0));
}

function renderNapAdminPanelHtml(user) {
  const alliances = Store.alliances;
  const napMap = Store.napAlliances;
  ...
  return `
    ...
    ${
      napAdminSubTab !== "rules"
        ...
        : `
    <div class="panel">
      <div class="planner-header"><strong>Current NAP Rules</strong></div>
      <p style="font-size:11px;color:var(--text-dim);margin-top:-6px;">Plain text — numbered lines, bullets, line breaks, and emoji are all fine as typed.</p>
      <textarea id="napRulesText" style="width:100%;background:var(--panel-2);border:1px solid var(--border);color:var(--text);border-radius:4px;padding:10px;font-size:13px;min-height:180px;resize:vertical;">${escapeHtml(Store.napRules)}</textarea>
      <button class="btn small primary" id="napRulesSave" style="margin-top:10px;">Save Rules</button>
      <span id="napRulesMsg" style="margin-left:10px;font-size:12px;color:var(--accent-green);"></span>
    </div>`
    }

    ${
      napAdminSubTab !== "alliances"
        ? ""
        : `
    <div class="panel">
      <div class="planner-header"><strong>NAP Alliances</strong></div>
      <p style="font-size:11px;color:var(--text-dim);margin-top:-6px;">Rank is calculated automatically from Power — highest Power is Rank 1. Power accepts "12.5B", "850M", or a plain number.</p>
      <div style="overflow-x:auto;">
        <table>
          <thead><tr><th>RANK</th><th>TAG</th><th>NAP</th><th>POWER</th><th>MEMBERS</th></tr></thead>
          <tbody>
            ${
              alliances
                .map((a) => {
                  const rec = napMap[a] || { isNap: false, power: 0, powerDisplay: "", members: 0 };
                  const rank = rec.isNap ? ranked.findIndex((r) => r.tag === a) + 1 : null;
                  return `
              <tr>
                <td>${rank ? "#" + rank : "—"}</td>
                <td>${allianceBadgeHtml(a)} ${escapeHtml(a)}</td>
                <td><input type="checkbox" data-napflag="${escapeHtml(a)}" ${rec.isNap ? "checked" : ""} /></td>
                <td><input data-nappower="${escapeHtml(a)}" value="${escapeHtml(rec.powerDisplay || "")}" placeholder="e.g. 12.5B" style="width:100px;background:var(--panel-2);border:1px solid var(--border);color:var(--text);border-radius:3px;padding:5px 8px;font-size:12px;" /></td>
                <td><input data-napmembers="${escapeHtml(a)}" type="number" min="0" value="${rec.members || ""}" style="width:80px;background:var(--panel-2);border:1px solid var(--border);color:var(--text);border-radius:3px;padding:5px 8px;font-size:12px;" /></td>
              </tr>`;
                })
                .join("") || `<tr><td colspan="5">No alliances yet — add one in Admin → Alliances first.</td></tr>`
            }
          </tbody>
        </table>
      </div>
    ...
    ${napAdminSubTab !== "fortress-current" ? "" : renderNapAdminSignupSectionHtml("FORTRESS", "PENDING", alliances)}
    ${napAdminSubTab !== "fortress-history" ? "" : renderNapAdminSignupSectionHtml("FORTRESS", "HISTORY", alliances)}
    ${napAdminSubTab !== "stronghold-current" ? "" : renderNapAdminSignupSectionHtml("STRONGHOLD", "PENDING", alliances)}
    ${napAdminSubTab !== "stronghold-history" ? "" : renderNapAdminSignupSectionHtml("STRONGHOLD", "HISTORY", alliances)}
  `;
}

// Shared by Fortress and Stronghold — identical structure, two completely
// separate Store arrays (napFortress / napStronghold), never mixed in one
// list. `statusView` is "PENDING" (current) or "HISTORY" (OBTAINED /
// NOT_OBTAINED) — which list a record shows in is driven purely by its own
// `status`, never a manual move.
function renderNapAdminSignupSectionHtml(type, statusView, alliances) {
  const storeKey = type === "FORTRESS" ? "napFortress" : "napStronghold";
  const all = Store[storeKey].map(normalizeNapSignup);
  const rows = statusView === "PENDING" ? all.filter((r) => r.status === "PENDING") : all.filter((r) => r.status !== "PENDING");
  const label = type === "FORTRESS" ? "Fortress" : "Stronghold";
  const prefix = type === "FORTRESS" ? "fort" : "sh";

  return `
    <div class="panel">
      <div class="planner-header"><strong>${statusView === "PENDING" ? `Current ${label} Sign Ups` : `Previous ${label} Sign Ups`} (${rows.length})</strong></div>
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:${statusView === "PENDING" ? "12px" : "0"};">
        ${
          rows.length
            ? rows
                .map(
                  (r) => `
          <div style="background:var(--panel-2);border:1px solid var(--border);border-radius:4px;padding:10px 12px;">
            <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:6px;">
              <strong style="font-size:12.5px;">${escapeHtml(r.locationName)}</strong>
              <span class="status-badge ${r.status === "OBTAINED" ? "done" : r.status === "NOT_OBTAINED" ? "open" : "planned"}">${r.status.replace("_", " ")}</span>
              ${allianceBadgeHtml(r.selectedAllianceTag)} <span style="font-size:11.5px;color:var(--text-dim);">${escapeHtml(r.selectedAllianceTag)}</span>
              <span style="font-size:10.5px;color:var(--text-faint);">signed up ${escapeHtml(r.signupDate)}</span>
            </div>
            ${
              statusView === "PENDING"
                ? `
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
              <button data-napobtained="${r.id}" data-nptype="${storeKey}" class="btn small primary">Obtained ✓</button>
              <select data-napnotobtained="${r.id}" data-nptype="${storeKey}" style="background:var(--panel);border:1px solid var(--border);color:var(--text);border-radius:3px;padding:5px 8px;font-size:11.5px;">
            ...
            <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;justify-content:space-between;">
              ...
              <div style="display:flex;gap:8px;flex-wrap:wrap;">
                ...
                .join("")
            : `<div class="empty">${statusView === "PENDING" ? `No current ${label.toLowerCase()} sign ups.` : `No ${label.toLowerCase()} history yet.`}</div>`
        }
      </div>
      ${
        statusView !== "PENDING"
          ...
          : `
      <div class="field-row">
        <div class="field"><label>${label.toUpperCase()}</label><input id="${prefix}Location" placeholder="e.g. ${label} 2" /></div>
        <div class="field">
          <label>SELECTED ALLIANCE</label>
          <select id="${prefix}Alliance">
            <option value="">Select alliance…</option>
            ${alliances.map((a) => `<option value="${escapeHtml(a)}">${escapeHtml(a)}</option>`).join("")}
          </select>
        </div>
        <div class="field"><label>SIGNUP DATE</label><input id="${prefix}Date" type="date" /></div>
      </div>
      <button class="btn small primary" id="${prefix}Add">Add ${label} Sign Up</button>`
      }
      ${
        statusView !== "HISTORY" || !rows.length
          ? ""
          : `<button data-napclearhist="${storeKey}" class="btn small" style="color:var(--accent-red);margin-top:12px;">Clear ${label} History</button>`
      }
    </div>
  `;
}
...
function wireNapAdminPanel(el, user) {
  el.querySelectorAll("[data-napsubtab]").forEach((btn) =>
    btn.addEventListener("click", () => {
      napAdminSubTab = btn.dataset.napsubtab;
      renderAdmin(el);
    })
  );

  el.querySelector("#napRulesSave")?.addEventListener("click", () => {
    Store.napRules = el.querySelector("#napRulesText").value;
    const msg = el.querySelector("#napRulesMsg");
    if (msg) msg.textContent = "Saved.";
  });

  el.querySelectorAll("[data-napflag]").forEach((cb) =>
    cb.addEventListener("change", () => {
      const tag = cb.dataset.napflag;
      const all = Store.napAlliances;
      all[tag] = { ...(all[tag] || { power: 0, powerDisplay: "", members: 0 }), isNap: cb.checked };
      Store.napAlliances = all;
      renderAdmin(el);
    })
  );
  el.querySelectorAll("[data-nappower]").forEach((inp) =>
    inp.addEventListener("change", () => {
      const tag = inp.dataset.nappower;
      const parsed = parsePowerToken(inp.value);
      const all = Store.napAlliances;
      all[tag] = { ...(all[tag] || { isNap: false, members: 0 }), powerDisplay: inp.value.trim(), power: parsed == null ? 0 : parsed };
      Store.napAlliances = all;
      renderAdmin(el);
    })
  );
  el.querySelectorAll("[data-napmembers]").forEach((inp) =>
    inp.addEventListener("change", () => {
      const tag = inp.dataset.napmembers;
      const all = Store.napAlliances;
      all[tag] = { ...(all[tag] || { isNap: false, power: 0, powerDisplay: "" }), members: Number(inp.value) || 0 };
      Store.napAlliances = all;
      renderAdmin(el);
    })
  );

  // Fortress / Stronghold — add, obtain, not-obtained. Shared handlers
  // (data-nptype carries which Store array — "napFortress"/"napStronghold").
  ["fort", "sh"].forEach((prefix) => {
    const storeKey = prefix === "fort" ? "napFortress" : "napStronghold";
    el.querySelector(`#${prefix}Add`)?.addEventListener("click", () => {
      const locationName = el.querySelector(`#${prefix}Location`).value.trim();
      const selectedAllianceTag = el.querySelector(`#${prefix}Alliance`).value;
      const signupDate = el.querySelector(`#${prefix}Date`).value;
      if (!locationName || !selectedAllianceTag) return;
      const list = Store[storeKey];
      Store[storeKey] = [
        ...list,
        { id: prefix + Date.now(), locationName, selectedAllianceTag, signupDate, status: "PENDING", obtained: false, takenByAllianceTag: null, notes: "", completedAt: null },
      ];
      renderAdmin(el);
    });
  });
  el.querySelectorAll("[data-napobtained]").forEach((btn) =>
    btn.addEventListener("click", () => {
      const storeKey = btn.dataset.nptype;
      const list = Store[storeKey];
      const idx = list.findIndex((r) => r.id === btn.dataset.napobtained);
      if (idx === -1) return;
      list[idx] = { ...list[idx], status: "OBTAINED", obtained: true, takenByAllianceTag: null, completedAt: Date.now() };
      Store[storeKey] = list;
      renderAdmin(el);
    })
  );
  el.querySelectorAll("[data-napnotobtained]").forEach((sel) =>
    sel.addEventListener("change", () => {
      if (!sel.value) return;
      const storeKey = sel.dataset.nptype;
      const list = Store[storeKey];
      const idx = list.findIndex((r) => r.id === sel.dataset.napnotobtained);
      if (idx === -1) return;
      list[idx] = { ...list[idx], status: "NOT_OBTAINED", obtained: false, takenByAllianceTag: sel.value, completedAt: Date.now() };
      Store[storeKey] = list;
      renderAdmin(el);
    })
  );
  ...
  el.querySelectorAll("[data-napclearhist]").forEach((btn) =>
    btn.addEventListener("click", () => {
      if (user.role !== "admin") return;
      const storeKey = btn.dataset.napclearhist;
      const label = storeKey === "napFortress" ? "Fortress" : "Stronghold";
      if (!confirm(`Are you sure you want to permanently delete ALL ${label} history? This cannot be undone.`)) return;
      // Keep PENDING (current sign-ups) — only completed (OBTAINED /
      // NOT_OBTAINED) history records are cleared, and only for this one
      // type; the other type's history is never touched by this button.
      Store[storeKey] = Store[storeKey].filter((r) => normalizeNapSignup(r).status === "PENDING");
      renderAdmin(el);
    })
  );
}

// Read-only NAP body — shared by the public "/nap" route (renderNapDashboard
// below) and the "NAP DASHBOARD" top-level section inside Admin for every
// signed-in role that isn't true ADMIN (LEADER/R4 land here read-only too,
// same as a plain MEMBER — only ADMIN gets renderNapAdminPanelHtml's edit
// controls). Order per spec: rules, rankings, current fortress, current
// stronghold, previous fortress, previous stronghold (history sections
// collapsible via <details>).
function renderNapDashboardBodyHtml() {
  const ranked = napRankedAlliances();
  const fortress = Store.napFortress.map(normalizeNapSignup);
  const stronghold = Store.napStronghold.map(normalizeNapSignup);
  const renderSignupRow = (r) => `
    <div style="background:var(--panel-2);border:1px solid var(--border);border-radius:4px;padding:10px 12px;margin-bottom:8px;">
      ...
      <div style="font-size:11px;color:var(--text-faint);">
        signed up ${escapeHtml(r.signupDate) || "—"}
        ${r.status === "NOT_OBTAINED" && r.takenByAllianceTag ? ` · taken by <strong style="color:var(--text-dim);">${escapeHtml(r.takenByAllianceTag)}</strong>` : ""}
        ${r.notes ? ` · ${escapeHtml(r.notes)}` : ""}
      </div>
    </div>`;
  ...
  return `
    <div class="panel">
      <div class="planner-header"><strong>Current NAP Rules</strong></div>
      <div style="font-size:12.5px;color:var(--text-dim);white-space:pre-wrap;overflow-wrap:anywhere;">${Store.napRules ? escapeHtml(Store.napRules) : `<span class="empty" style="padding:0;">No rules published yet.</span>`}</div>
    </div>

    <div class="panel">
      <div class="planner-header"><strong>NAP Alliance Rankings</strong></div>
      <div style="overflow-x:auto;">
        <table>
          <thead><tr><th>RANK</th><th>ALLIANCE</th><th>POWER</th><th>MEMBERS</th></tr></thead>
          <tbody>
            ${
              ranked
                .map(
              ...
              <tr>
                <td>#${i + 1}</td>
                <td>${allianceBadgeHtml(r.tag)} ${escapeHtml(r.tag)}</td>
                <td>${escapeHtml(r.powerDisplay || fmtNum(r.power))}</td>
                <td>${r.members || 0}</td>
              </tr>`
                )
                .join("") || `<tr><td colspan="4">No NAP alliances configured yet.</td></tr>`
            }
          </tbody>
        </table>
      </div>
    ...
    <div class="panel">
      <div class="planner-header"><strong>Current Fortress Sign Ups</strong></div>
      ${fortress.filter((r) => r.status === "PENDING").map(renderSignupRow).join("") || `<div class="empty">No current fortress sign ups.</div>`}
    </div>

    <div class="panel">
      <div class="planner-header"><strong>Current Stronghold Sign Ups</strong></div>
      ${stronghold.filter((r) => r.status === "PENDING").map(renderSignupRow).join("") || `<div class="empty">No current stronghold sign ups.</div>`}
    </div>

    <details class="panel">
      <summary style="cursor:pointer;font-weight:700;font-size:13px;">Previous Fortress Sign Ups</summary>
      <div style="margin-top:10px;">
        ${fortress.filter((r) => r.status !== "PENDING").map(renderSignupRow).join("") || `<div class="empty">No fortress history yet.</div>`}
      </div>
    </details>

    <details class="panel">
      <summary style="cursor:pointer;font-weight:700;font-size:13px;">Previous Stronghold Sign Ups</summary>
      <div style="margin-top:10px;">
        ${stronghold.filter((r) => r.status !== "PENDING").map(renderSignupRow).join("") || `<div class="empty">No stronghold history yet.</div>`}
      </div>
    </details>
  `;
}

// Public "/nap" route — full-page wrapper around renderNapDashboardBodyHtml
// above, plus the sign-in/access gate. Reachable by every signed-in user
// (see napDashboardAccessible) — read-only for everyone; only ADMIN's own
// "NAP DASHBOARD" section inside Admin has edit controls.
function renderNapDashboard(el) {
  const user = Store.currentUser;
